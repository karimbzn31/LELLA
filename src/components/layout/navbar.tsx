...
import { Menu, X, Search, Heart, Bell, User, ChevronDown } from "lucide-react";
    ...
    >
      <nav className="section-container flex items-center justify-between h-16 md:h-20">
        ...
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="hidden sm:inline text-gold text-xl">✦</span>
        </Link>
          ...
          {isAuthenticated ? (
            <>
              <Link
                ...
                className="p-2 text-navy/50 hover:text-navy transition-colors relative"
                aria-label="Favoris"
              >
                <Heart size={20} />
              </Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link href="/auth/signup">
                <Button variant="gold" size="sm">Inscription</Button>
              </Link>
            </div>
          )}
        </div>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden bg-white/95 backdrop-blur-xl border-t border-sand/50"
          >
            <div className="section-container py-6 space-y-4">
              {NAV_LINKS.map((link) => (
                <Link
                  ...
                  className="block py-2 text-lg font-medium text-navy/70 hover:text-navy transition-colors"
                >
                  {link.label}
                </Link>
              ))}
                ...
                {isAuthenticated ? (
                  <>
                    <Link
                      href="/dashboard/client"
                      onClick={() => setMobileMenuOpen(false)}
                      className="block py-2 text-lg font-medium text-navy/70"
                    >
                      Tableau de bord
                    </Link>
                    <button
                      onClick={() => { logout(); setMobileMenuOpen(false); }}
                      className="block py-2 text-lg font-medium text-error"
                    >
                      Déconnexion
                    </button>
                  </>
                  ...
                  <div className="flex gap-3">
                    ...
                    <Link href="/auth/signup" className="flex-1" onClick={() => setMobileMenuOpen(false)}>
                      <Button variant="gold" className="w-full">Inscription</Button>
                    </Link>
                  </div>
                )}
              </div>
